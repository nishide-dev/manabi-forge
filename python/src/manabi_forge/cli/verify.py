...
def _record_review(material_dir: Path, report: VerificationReport) -> Path:
    ...
    record = ReviewRecord.model_validate(
        {
            ...
            "reviewed_commit": _current_commit(material_dir),
            "result": result,
            "findings": findings,
            "created_at": now,
        },
...
def math(
    ...
    if record:
        typer.echo(f"recorded {_record_review(material_dir, report)}")
